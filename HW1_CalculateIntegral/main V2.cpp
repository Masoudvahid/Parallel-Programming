...
void MpiSend(int rank, integral_params integ_params) {
	MPI_Send(&integ_params.low_lim, 1, MPI_DOUBLE, rank, 1, MPI_COMM_WORLD);
	MPI_Send(&integ_params.step_size, 1, MPI_DOUBLE, rank, 2, MPI_COMM_WORLD);
	MPI_Send(&integ_params.steps_num, 1, MPI_DOUBLE, rank, 3, MPI_COMM_WORLD);
}

void MpiRecv(integral_params& splitted_params) {
	MPI_Recv(&splitted_params.low_lim, 1, MPI_DOUBLE, 0, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
	MPI_Recv(&splitted_params.step_size, 1, MPI_DOUBLE, 0, 2, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
...
int main() {
	...
	if (my_rank == 0) {
		/* ----------------- Calculate the integral using LINEAR programming ----------------- */
		/*integ_params.low_lim = integ_info.low_lim;
		integ_params.step_size = integ_info.up_lim / integ_info.sub_intervals;
		integ_params.steps_num = integ_info.sub_intervals;
		std::cout << "Integral answer using linear programming = "
			<< CalcIntegral(integ_params) << "\n\n\n";*/


			/* ----------------- Calculate the integral using PARALLEL programming ----------------- */

			/* calculate integral for interval part of rank = 0 */
		double jump = (int)integ_info.sub_intervals / world_size;
		int jump_residue = (int)integ_info.sub_intervals % world_size;

		//double new_lower_limit = integ_params.low_lim;
		...
		/* calculate integral for interval part of rank != 0 */
		int rank = 1;
		for (int i = jump_residue; i > 0; i--)
		{
			integ_params.steps_num = jump + 1;

			MpiSend(rank, integ_params);

			integ_params.low_lim += (jump + 1) * new_step_size;
			rank++;
		}
	...
	if (my_rank != 0) {
		MpiRecv(splitted_params);
		PrintOutput(my_rank, splitted_params);
